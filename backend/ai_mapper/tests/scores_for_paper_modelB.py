...
def inner(w, k):
    ...
    for i, row in df.iterrows():
        ...
        response = requests.get(f"http://localhost:8000/v2/ai-mapper?variable_name={var_2}&variable_description={description_2}&k={k}&w_1={1-w}&w_2={w}&bypass_binary=false&class_prob_threshold=0.0&usePriorMappings=false")
        response_json = response.json()
        prediction = list(response_json["class_props"].keys())
        
        if prediction == "No suitable variable found." and description_2 != "":
            response = requests.get(f"http://localhost:8000/v2/ai-mapper?variable_name=&variable_description={description_2}&k={k}&w_1={1-w}&w_2={w}&bypass_binary=false&class_prob_threshold=0.0&usePriorMappings=false")
            response_json = response.json()
            prediction = list(response_json["class_props"].keys())
        if prediction == "No suitable variable found.":
            response = requests.get(f"http://localhost:8000/v2/ai-mapper?variable_name={var_2}&variable_description=&k={k}&w_1={1-w}&w_2={w}&bypass_binary=false&class_prob_threshold=0.0&usePriorMappings=false")
            response_json = response.json()
            prediction = list(response_json["class_props"].keys())
        
        if var_1.lower().strip() in prediction:
            correct += 1
